...
from models.sac_agent_model import SACAgent  # SAC implementation
...
def train_agent(data_path, model_path, num_epochs=50):
    ...
    df["usage_norm"] = (df["usage"] - df["usage"].min()) / (
    ...
    )  # action_dim=1 for continuous scalar
    ...
    for epoch in range(num_epochs):
        ...
        for idx, row in df.iterrows():
            ...
            action = agent.select_action(
                state
            )  # action is continuous scalar, e.g., in [0,1]
