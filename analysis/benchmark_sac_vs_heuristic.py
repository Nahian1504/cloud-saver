...
def benchmark(model_path, aws_data_path, interruption_data_path):
    ...
    # Normalize features
    df["norm_cost"] = (df["cost"] - df["cost"].min()) / (
        df["cost"].max() - df["cost"].min()
    )
    df["norm_usage"] = (df["usage"] - df["usage"].min()) / (
    ...
    state_size = 4
    agent = SACAgent(
        state_dim=state_size, action_dim=1
    )  # action_dim=1 for continuous action
    ...
    for _, row in df.iterrows():
        ...
        state = np.array(
            [row["norm_cost"], norm_usage, hour / 23.0, day / 6.0], dtype=np.float32
        )
        ...
        sac_penalty = (
            penalty_function(overall_interruption_rate) if sac_choice == "spot" else 0
        )
        ...
        heuristic_penalty = (
            penalty_function(overall_interruption_rate)
            if heuristic_choice == "spot"
            else 0
        )
    ...
    avg_heuristic_reward = full_df[full_df["policy_type"] == "Heuristic"][
        "reward"
    ].mean()
    avg_heuristic_penalty = full_df[full_df["policy_type"] == "Heuristic"][
        "penalty"
    ].mean()
    ...
    logger.info(
        f"Avg Heuristic Reward: {avg_heuristic_reward:.4f}, Penalty: {avg_heuristic_penalty:.4f}"
    )

    print(f"[SAC] Avg Reward: {avg_sac_reward:.2f}, Avg Penalty: {avg_sac_penalty:.3f}")
    print(
        f"[Heuristic] Avg Reward: {avg_heuristic_reward:.2f}, Avg Penalty: {avg_heuristic_penalty:.3f}"
    )
