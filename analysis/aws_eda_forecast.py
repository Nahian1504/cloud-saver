...
df_hybrid = load_data(os.path.join(data_dir, 'aws_hybrid_usage.csv'), ['usage_start_time'])
...
def plot_cost_trend(df, date_col, title, filename):
    ...
    if 'data_type' in df.columns:
        ...
        for dtype, color, label in [('real', 'blue', 'Real'), ('synthetic', 'orange', 'Synthetic')]:
            if dtype in df['data_type'].values:
                subset = df[df['data_type'] == dtype]
                sns.lineplot(data=subset, x=date_col, y='cost', 
                            marker='o', ax=ax, color=color, label=label)
    else:
...
def forecast_cost(df, date_col, title, filename):
    ...
    try:
        # Data preparation
        df_clean = df[[date_col, 'cost']].dropna().rename(columns={date_col: 'ds', 'cost': 'y'})
...
if df_real is not None and len(df_real) >= 2:
    plot_cost_trend(df_real, 'date', 'Real AWS Cost Trend', 
                   os.path.join(visuals_dir, 'real_cost_trend.png'))
    forecast_cost(df_real, 'date', 'Real AWS Cost',
                 os.path.join(visuals_dir, 'real_cost_forecast.png'))

if df_hybrid is not None:
    plot_cost_trend(df_hybrid, 'usage_start_time', 'Hybrid AWS Cost Trend', 
                   os.path.join(visuals_dir, 'hybrid_cost_trend.png'))
    forecast_cost(df_hybrid, 'usage_start_time', 'Hybrid AWS Cost',
