...
def analyze_temporal_patterns(df):
    ...
    if len(daily_costs) < 14:  # Insufficient for seasonal_decompose
        logger.warning(
            f"Insufficient data ({len(daily_costs)} points). Using fallback patterns."
        )
        ...
        seasonal_pattern = [
            ...
            1.2,
            0.8,
        ]  # Typical AWS weekly fluctuation
        ...
        return {
            ...
            "residual_std": (
                daily_costs["cost"].std() * 0.3 if len(daily_costs) > 1 else 2.0
            ),
        }
    else:
...
def generate_hybrid_data(real_df, num_days=30):
    ...
    last_real_date = (
        real_df["date"].max() if not real_df.empty else datetime.date.today()
    )
    synth_dates = pd.date_range(
        start=last_real_date + datetime.timedelta(days=1), periods=num_days
    )

    # 4. Create baseline trend
    last_trend = patterns["trend"].iloc[-1] if not patterns["trend"].empty else 10.0
    trend_values = np.linspace(
        last_trend, last_trend * 1.15, num_days
    )  # 15% monthly increase

    # 5. Generate synthetic data
    hybrid_data = []
    services = (
        real_df["service"].unique() if not real_df.empty else ["AmazonEC2", "AmazonS3"]
    )

    for i, date in enumerate(synth_dates):
        # Get seasonal component
        seasonal_idx = (
            date.dayofweek % len(patterns["seasonal"])
            if not patterns["seasonal"].empty
            else 0
        )
        seasonal_factor = (
            patterns["seasonal"].iloc[seasonal_idx]
            if not patterns["seasonal"].empty
            else 1.0
        )
        ...
        for service in services:
            ...
            usage = round(
                base_usage * (0.9 + 0.2 * random.random()), 2
            )  # ±10% variation
...
def generate_validation_report(hybrid_df):
    ...
    if "data_type" in hybrid_df.columns:
        real = (
            hybrid_df[hybrid_df["data_type"] == "real"]["cost"]
            if "real" in hybrid_df["data_type"].values
            else pd.Series()
        )
