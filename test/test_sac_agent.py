...
def test_agent_select_action_method(agent, test_data):
    ...
    for _, row in test_data.head(10).iterrows():  # limit to 10 for speed
        state = np.array(
            [
                (row["cost"] - test_data["cost"].min())
                / (test_data["cost"].max() - test_data["cost"].min()),
                (row["usage"] - test_data["usage"].min())
                / (test_data["usage"].max() - test_data["usage"].min()),
        ...
        logger.info(
            f"Evaluation action: {eval_action}, Exploration action: {explore_action}"
        )
...
def test_model_parameters_loaded(agent):
    ...
    policy_all_zeros = all(
        torch.equal(p.data, torch.zeros_like(p.data)) for p in policy_params
    )
...
def test_target_network_updates(agent):
    ...
    for target_param, param in zip(agent.q1_target.parameters(), agent.q1.parameters()):
        ...
        ), "Target network matches main network"
...
def test_replay_buffer_operation(agent):
    ...
    assert (
        len(agent.memory) > 0
    ), "Replay buffer should not be empty after adding experience"
