...
def load_data(file_path):
    ...
    try:
        ...
        date_range_info = ""
        if primary_date_col:
            date_range_info = f"Date range: {df[primary_date_col].min()} to {df[primary_date_col].max()}"
        else:
...
def validate_data(df):
    ...
    try:
        ...
        filename = [
            k for k in required_columns if k in str(df.attrs.get("filename", ""))
        ]
        filename = filename[0] if filename else None

        if filename and not all(
            col in df.columns for col in required_columns[filename]
        ):
            missing = [
                col for col in required_columns[filename] if col not in df.columns
            ]
...
def setup_logging():
    ...
    logging.basicConfig(
        filename=os.path.join(
            log_dir, f'utils_{datetime.now().strftime("%Y%m%d")}.log'
        ),
        level=logging.INFO,
